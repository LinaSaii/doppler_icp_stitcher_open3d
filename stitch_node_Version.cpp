...
public:
    DopplerICPStitcher() : Node("doppler_icp_stitcher"), 
        ...
        declare_parameter("frames_directory", "/home/nassim/test_lidar/data/indoor_sparse_short_scene/cc");
        ...
        declare_parameter("last_n_frames", std::vector<int>{1, 5, 10, 15, 20, 25, 30});        
        ...
        // NEW: SNR parameters
        declare_parameter("snr_history_size", std::vector<int>{50});
        declare_parameter("save_snr_csv", std::vector<bool>{true});
        declare_parameter("snr_csv_output", std::vector<std::string>{"./snr_data_indoor_sparce_short.csv"});
        ...
        // NEW: Initialize SNR CSV if enabled
        if (save_snr_csv_) {
            initialize_snr_csv();
        }
        ...
        // NEW: SNR KPI publisher
        snr_kpi_pub_ = create_publisher<std_msgs::msg::Float64>("snr_kpi", qos);
...
private:
    ...
    // NEW: SNR variables
    std::vector<double> snr_list_;
    double single_frame_snr_baseline_ = 0.0;
    int snr_history_size_ = 50;
    bool save_snr_csv_ = true;
    std::string snr_csv_output_;
    
    // NEW: SNR CSV logging
    std::ofstream snr_csv_file_;
    std::string snr_csv_filename_;
    ...
    void initialize_parameter_combinations() {
        ...
        // NEW: SNR parameters
        auto snr_history_sizes = get_parameter("snr_history_size").as_integer_array();
        auto save_snr_csv_list = get_parameter("save_snr_csv").as_bool_array();
        auto snr_csv_outputs = get_parameter("snr_csv_output").as_string_array();
        ...
        size_t max_size = std::max({
            ...
            snr_history_sizes.size(), save_snr_csv_list.size(), snr_csv_outputs.size()
        });
        ...
        // Set SNR parameters from first combination
        snr_history_size_ = snr_history_sizes[0];
        save_snr_csv_ = save_snr_csv_list[0];
        snr_csv_output_ = snr_csv_outputs[0];
    ...
    // NEW: Initialize SNR CSV file
    void initialize_snr_csv() {
        if (!save_snr_csv_) return;
        
        try {
            // Create directory if needed
            std::filesystem::path csv_path(snr_csv_output_);
            std::filesystem::create_directories(csv_path.parent_path());
            
            snr_csv_filename_ = snr_csv_output_;
            snr_csv_file_.open(snr_csv_filename_, std::ios::out | std::ios::trunc);
            
            if (!snr_csv_file_.is_open()) {
                RCLCPP_ERROR(get_logger(), "Failed to open SNR CSV file: %s", snr_csv_filename_.c_str());
                save_snr_csv_ = false;
                return;
            }
            
            // Write header
            snr_csv_file_ << "frame_index,timestamp,frame_filename,snr_gain_db,"
                         << "stitched_snr_db,baseline_snr_db,num_frames_in_memory,"
                         << "num_points_stitched,pose_x,pose_y,pose_z,"
                         << "pose_qx,pose_qy,pose_qz,pose_qw\n";
            snr_csv_file_.flush();
            
            RCLCPP_INFO(get_logger(), "Initialized SNR CSV output: %s", snr_csv_filename_.c_str());
            
        } catch (const std::exception& e) {
            RCLCPP_ERROR(get_logger(), "Failed to initialize SNR CSV: %s", e.what());
            save_snr_csv_ = false;
        }
    }
    ...
    // NEW: Write SNR data to CSV
    void write_snr_to_csv(size_t frame_idx, const std::string& frame_filename, 
                         double snr_gain_db, double stitched_snr_db, double baseline_snr_db,
                         size_t num_frames, size_t num_points) {
        if (!save_snr_csv_ || !snr_csv_file_.is_open()) return;
        ...
        try {
            ...
            Eigen::Vector3d position = current_pose_.block<3,1>(0,3);
            Eigen::Quaterniond quat(current_pose_.block<3,3>(0,0));
            
            snr_csv_file_ << std::fixed << std::setprecision(6);
            snr_csv_file_ << frame_idx << ","
                         << now_time_t << ","
                         << frame_filename << ","
                         << snr_gain_db << ","
                         << stitched_snr_db << ","
            ...
            RCLCPP_ERROR(get_logger(), "Failed to write SNR data to CSV: %s", e.what());
        }
    }
    ...
    void close_snr_csv() {
        if (snr_csv_file_.is_open()) {
            snr_csv_file_.close();
            RCLCPP_INFO(get_logger(), "SNR data saved to: %s", snr_csv_filename_.c_str());
        }
    }
    ...
    // NEW: SNR computation functions
    double compute_snr(const Eigen::MatrixXd& points) {
        if (points.rows() == 0) {
            return 0.0;
        }
        
        try {
            Eigen::MatrixXd points_sample;
            if (points.rows() > 2500000) {
                // Random sampling for large point clouds
                std::vector<int> indices(points.rows());
                std::iota(indices.begin(), indices.end(), 0);
                std::shuffle(indices.begin(), indices.end(), std::default_random_engine{});
                indices.resize(2500000);
                
                points_sample.resize(2500000, 3);
                for (size_t i = 0; i < indices.size(); ++i) {
                    points_sample.row(i) = points.row(indices[i]);
                }
            } else {
            ...
            pcd->points_.reserve(points_sample.rows());
            for (int i = 0; i < points_sample.rows(); ++i) {
                pcd->points_.push_back(points_sample.row(i).transpose());
            }
            
            // Estimate normals
            pcd->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(0.3, 30));
            
            // Build KDTree for nearest neighbors
            open3d::geometry::KDTreeFlann kdtree(*pcd);
            int k_neighbors = std::min(30, static_cast<int>(points_sample.rows()));
            
            std::vector<double> residuals;
            residuals.reserve(points_sample.rows());
            
            // Compute point-to-plane distances
            for (int i = 0; i < points_sample.rows(); ++i) {
                std::vector<int> indices_vec;
                std::vector<double> distances;
                kdtree.SearchKNN(pcd->points_[i], k_neighbors, indices_vec, distances);
                
                // Compute centroid of neighbors
                Eigen::Vector3d centroid = Eigen::Vector3d::Zero();
                for (int idx : indices_vec) {
                    centroid += pcd->points_[idx];
                }
                centroid /= indices_vec.size();
                
                // Compute deviation from centroid
                Eigen::Vector3d deviation = pcd->points_[i] - centroid;
                Eigen::Vector3d normal = pcd->normals_[i];
                double point_to_plane_dist = std::abs(deviation.dot(normal));
                residuals.push_back(point_to_plane_dist);
            }
            
            // Compute noise power (mean of squared residuals)
            double noise_power = 0.0;
            for (double r : residuals) {
                noise_power += r * r;
            }
            noise_power = noise_power / residuals.size() + 1e-12;
            
            // Compute signal power (variance of points)
            Eigen::Vector3d centroid_global = Eigen::Vector3d::Zero();
            for (int i = 0; i < points_sample.rows(); ++i) {
                centroid_global += points_sample.row(i).transpose();
            }
            centroid_global /= points_sample.rows();
            
            double signal_power = 0.0;
            for (int i = 0; i < points_sample.rows(); ++i) {
                Eigen::Vector3d diff = points_sample.row(i).transpose() - centroid_global;
                signal_power += diff.squaredNorm();
            }
            signal_power /= points_sample.rows();
            ...
            if (std::rand() % 10 == 0) {
                ...
                for (double r : residuals) std_residual += (r - mean_residual) * (r - mean_residual);
                std_residual = std::sqrt(std_residual / residuals.size());
                
                RCLCPP_DEBUG(get_logger(), 
                    "SNR Debug: signal=%.4f, noise=%.4f, mean_residual=%.4f, std_residual=%.4f",
                    signal_power, noise_power, mean_residual, std_residual);
            }
            
            return signal_power / noise_power;
            
        } catch (const std::exception& e) {
            RCLCPP_WARN(get_logger(), "SNR computation failed: %s, using fallback", e.what());
            ...
            double signal_power = 0.0;
            for (int i = 0; i < points.rows(); ++i) {
                Eigen::Vector3d diff = points.row(i).transpose() - centroid;
                signal_power += diff.squaredNorm();
            }
            signal_power /= points.rows();
            
            // Simple noise estimation using average distance to k-nearest neighbors
            auto pcd = std::make_shared<open3d::geometry::PointCloud>();
            pcd->points_.reserve(points.rows());
            for (int i = 0; i < points.rows(); ++i) {
                pcd->points_.push_back(points.row(i).transpose());
            }
            
            open3d::geometry::KDTreeFlann kdtree(*pcd);
            int k = std::min(10, static_cast<int>(points.rows()));
            double noise_power = 0.0;
            int count = 0;
            ...
            for (int i = 0; i < points.rows(); ++i) {
                ...
                std::vector<double> distances;
                if (kdtree.SearchKNN(pcd->points_[i], k, indices, distances) > 0) {
                    for (size_t j = 1; j < distances.size(); ++j) { // Skip self
                        noise_power += distances[j] * distances[j];
                        count++;
                    }
                }
            }
            
            noise_power = (count > 0) ? (noise_power / count) + 1e-12 : 1e-12;
            return signal_power / noise_power;
        }
    }
    
    double compute_snr_db(const Eigen::MatrixXd& points) {
        double snr_linear = compute_snr(points);
        if (snr_linear <= 0) {
    ...
    double compute_snr_gain_db(double stitched_snr_db, double baseline_snr_db) {
        return stitched_snr_db - baseline_snr_db;
}

    // NEW: Publish SNR KPI
    void publish_snr_kpi(double snr_value) {
        auto snr_msg = std_msgs::msg::Float64();
        snr_msg.data = snr_value;
        snr_kpi_pub_->publish(snr_msg);
        
        // NEW: Record to MCAP if enabled
        record_message(snr_msg, "snr_kpi");
    }
    ...
    void process_next_frame() {
        ...
        if (!previous_frame_set_) {
            ...
            // NEW: SNR baseline computation for first frame
            single_frame_snr_baseline_ = compute_snr(frame_data.points);
            double baseline_snr_db = compute_snr_db(frame_data.points);
            
            RCLCPP_INFO(get_logger(), 
                "Baseline single-frame SNR: %.2f dB (linear: %.4f)", 
                baseline_snr_db, single_frame_snr_baseline_);
            
            snr_list_.push_back(0.0);
            publish_snr_kpi(0.0);
            
            // Write to CSV
            write_snr_to_csv(frame_idx_, 
                            fs::path(frame_files_[frame_idx_]).filename().string(),
                            0.0, baseline_snr_db, baseline_snr_db,
                            1, frame_data.points.rows());
            ...
            // NEW: SNR computation for stitched point cloud
            Eigen::MatrixXd all_points_for_snr;
            
            // Collect all transformed points for SNR computation
            Eigen::Matrix4d current_pose_inv = current_pose_.inverse();
            std::vector<Eigen::MatrixXd> transformed_frames;
            
            for (const auto& [frame_pts, pose] : stitched_frames_) {
                ...
                Eigen::MatrixXd transformed = (R * frame_pts.points.transpose()).colwise() + t;
                transformed_frames.push_back(transformed.transpose());
            }
            
            // Combine all points
            size_t total_points = 0;
            for (const auto& frame : transformed_frames) {
                total_points += frame.rows();
            }
            
            all_points_for_snr.resize(total_points, 3);
            size_t current_row = 0;
            for (const auto& frame : transformed_frames) {
                all_points_for_snr.block(current_row, 0, frame.rows(), 3) = frame;
                current_row += frame.rows();
            }
            
            // Compute SNR
            double current_snr_linear = compute_snr(all_points_for_snr);
            double current_snr_db = compute_snr_db(all_points_for_snr);
            double baseline_snr_db = compute_snr_db(frame_data.points);
            
            double snr_gain_db = compute_snr_gain_db(current_snr_db, baseline_snr_db);
            
            snr_list_.push_back(snr_gain_db);
            if (snr_list_.size() > static_cast<size_t>(snr_history_size_)) {
                snr_list_.erase(snr_list_.begin());
            }
            
            RCLCPP_INFO(get_logger(),
                "Frame %zu: Stitched SNR = %.2f dB, SNR Gain = %.2f dB",
                frame_idx_, current_snr_db, snr_gain_db);
            
            // Write to CSV
            write_snr_to_csv(frame_idx_,
                            fs::path(frame_files_[frame_idx_]).filename().string(),
                            snr_gain_db, current_snr_db, baseline_snr_db,
                            stitched_frames_.size(), all_points_for_snr.rows());
            
            publish_snr_kpi(snr_gain_db);
    ...
    // NEW: SNR KPI publisher
    rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr snr_kpi_pub_;
